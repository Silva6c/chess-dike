...
> 版本：versionName 2.5 · 包名 `com.xqdk.chess` · 构建：Gradle 8.10 / AGP（JDK 17）
...
象棋迪克 = **完整对弈/打谱**（DroidFish 衍生基座 + Pikafish 引擎）+ **连线**（自研：
...
├─ com.xqdk.chess.gamelogic         # 棋规引擎（Board/Rule/Move/Zobrist，源自基座）
├─ com.xqdk.chess.controllers       # 对弈/打谱控制器（DroidFish 状态机）
├─ com.xqdk.chess.openbook          # 开局库（SQLite 桔库 + Zobrist 查询）
├─ com.xqdk.chess.manuals           # XQF 棋谱解析（GB18030 + 解密）
